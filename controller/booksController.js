...
module.exports = {
  ...
  create : function(req,res){
    const obj = req.body;
    Book.create(obj, function (err, instance) {
      if(!err){
        res.status(201).json({
  ...
  remove : function(req,res){
    Book.deleteOne({_id: new mongodb.ObjectID(req.params.id)},(err)=>{
      if(!err){
  ...
  update : function(req,res){
    const data = req.body;
    Book.updateOne({_id: new mongodb.ObjectID(req.params.id)},req.body,(err)=>{
      if(!err){
        res.status(200).json({
